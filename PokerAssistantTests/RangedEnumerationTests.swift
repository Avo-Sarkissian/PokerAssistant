...
struct RangeRoutingTests {
    ...
    /// The inference is only worth anything if it survives the routing. Postflop
    /// heads-up goes to exact enumeration, which used to discard the range entirely.
    @Test("Postflop equity reflects the opponent range the app inferred")
    func postflopEquityUsesTheRange() async {
        ...
        #expect(versusTight < versusRandom - 0.02,
                "tight \(versusTight) vs random \(versusRandom) — the range is being discarded")
    }
}
